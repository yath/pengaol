...
while ((nTrameLen=m_CKernel->ReadAolTrame(sTrame,(MRUIn+150)))<6);
...
if (!AolHeader->IsValid())
	...
	nLon=m_CKernel->HexToSeq(sBuffer,(char *) m_CKernel->CData->dat19,AolHeader->GetSeq(),AolHeader->GetAck());
	m_CKernel->WriteRawIn(sBuffer,nLon);
	// Desactive les trames decouper
	nPosTmpBuffer=0;
	m_CKernel->m_cMsgIn->Printf("%LErreur de Checksum : demande de la trame %d : %d \n",nLastAck,nLastSeq);
	// Et on cherche une trame valide
	while (!AolHeader->IsValid())
		{
		while (m_CKernel->ReadAolTrameSeqAck(sTrame,MRUIn,nLastSeq,nLastAck)<7);
		AolHeader->SetHeader(sTrame);
		AolHeader->SetNormal();
       }
	}

// A partir d'ici on est sur d'avoir a faire a une trame AOL
...
switch (AolHeader->GetTyp())
	{
	case 0x26 : // Ping - Pong
							nLon=m_CKernel->HexToSeq(sBuffer,(char *) m_CKernel->CData->dat16,nLastAck,nLastSeq);
							m_CKernel->WriteRawIn(sBuffer,nLon);
							bNeedRestart=true;
							break;
...
switch (AolHeader->GetCode())
   ...
   case 0x6174:   //trame aim
								Aim->Buddy((unsigned char *) &ssTrame[0],AolHeader->GetTotalSize()+2,LastCode);
								bNeedRestart=true;
								break;
   ...
   case 0x7964:		// Trame Aol Decoupe
									...
									else
								{
									...
									memcpy((sTmpBuffer+nPosTmpBuffer),AolHeader->GetNextPayload(),AolHeader->GetTotalSize()-5);
									pPayload=sTmpBuffer;
									nPayloadLen=(nPosTmpBuffer+AolHeader->GetTotalSize()-5);
									nPosTmpBuffer=0;
									bNeedRestart=false;
								}
								break;
