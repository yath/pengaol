...
if (FileExist(sSource))
	...
	if ((nEntree=open(sSource,O_RDONLY))>0)
		if ((nSortie=open(sDestination,O_WRONLY | O_CREAT,S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH))>0)
			{
...
if ((!bDin) || (!bDout))
   ...
   if (!m_nErrorNbr)
		...
		if (GuidIn==Modem)	bRet=RouteModem();
		if (GuidIn==Cable)		bRet=RouteCable();

		if (!bRet) m_nErrorNbr=70;
		}
	}
...
FileCopy(sFile,"restmp.tmp");
unlink(sFile);

fFichier=fopen(sFile,"w");
fprintf(fFichier,"%s",sTexte);
fclose(fFichier);

fFile=fopen("restmp.tmp","r");
...
if ((!bDin) || (!bDout))
	...
	if (!FileCopy(Source,Destination))
      ...
      if (!m_nErrorNbr)
		{
		if (GuidIn==Modem)	bRet=DelRouteModem();
		if (GuidIn==Cable)		bRet=DelRouteCable();

		if (!bRet) m_nErrorNbr=73;
		}
	}
...
/** action ifconfig */
int CRouteurLinux::sifaddr (int unit, u_int32_t our_adr, u_int32_t his_adr,u_int32_t net_mask)
{
   struct ifreq   ifr;
    struct rtentry rt;
 ...
 */
	if (!get_ether_addr((unsigned int) his_adr, (sockaddr *) &arpreq.arp_ha, (char *) proxy_arp_dev,
...
{
//sprintf(sbuf,"route add -net %s gw %s netmask %s",sSource,m_sIp,sNetmask);
//system(sbuf);
}
/** cherche /proc */
...
int CRouteurLinux::get_ether_addr (u_int32_t ipaddr,       struct sockaddr *hwaddr,       char *name, int namelen)
{
...
/** routage par defaut */
int CRouteurLinux::sifdefaultroute (int unit, u_int32_t ouraddr, u_int32_t gateway)
{
   struct rtentry rt;
...
int CRouteurLinux::cifdefaultroute (int unit, u_int32_t ouraddr, u_int32_t gateway)
{
    struct rtentry rt;
