...
if (!(m_CKernel->m_cDriverIn->IsConnected()) || !(m_CKernel->m_cDriverIn->IsConnected()))
	Exit(0xff);
...
if (m_CKernel->bNeedAck)	
	{
	nLoop++;
	if ((nSaveAck!=m_CKernel->m_nNeedAck) || (nSaveSeq!=m_CKernel->m_nNeedSeq))
		{
...
switch (nDiv)
	...
	case 5:
	case 7:
	case 8: 	nLon=m_CKernel->HexToSeq(sBuffer,(char *) m_CKernel->CData->dat17,nSaveSeq,nSaveAck);
					m_CKernel->WriteRawIn(sBuffer,nLon);	
					
					break;
	case 3:
	case 6:
	case 9:	nLon=m_CKernel->HexToSeq(sBuffer,(char *) m_CKernel->CData->dat17,nSaveSeq,nSaveAck);
					m_CKernel->WriteRawIn(sBuffer,nLon);
                nLon=m_CKernel->HexToSeq(sBuffer,(char *) m_CKernel->CData->dat19,nSaveSeq,nSaveAck);
					m_CKernel->WriteRawIn(sBuffer,nLon);
					break;
