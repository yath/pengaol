...
// recherche les elements
	...
	while (fgets((char *) &cBuffer,100,fFifd)!=NULL)
		...
		if (strstr((char*) &cBuffer,(char *) &cNameClass)!=NULL) bStartFind=true;
		if (bStartFind==true)  CheckParam((char *) &cBuffer);
		}
	fclose(fFifd);
	bRet=true;
	}
	...
	*pPosEgal='\0';
	...
	while (m_sParam[nParamNbr].Request!=NULL)
		...
		if (strcmp(cBuffer,m_sParam[nParamNbr].Request)==0)  //Bingo
    		...
    		if (nLon>m_sParam[nParamNbr].MaxBufferLen) nLon=m_sParam[nParamNbr].MaxBufferLen;
				{
				memcpy(m_sParam[nParamNbr].InitStr,pPosEgal,nLon);
				// suprime \n
				if ((pPosEgal=strstr(m_sParam[nParamNbr].InitStr,(char *) "\n"))!=NULL) *pPosEgal='\0';
				break;
				}
			}	
		nParamNbr++;	
...
if ((fFifd=fopen((const char*) m_sFile,"a"))==NULL)
	...
	while (Param[nParamNbr].Request!=NULL)
		...
		if (nParamNbr==0)
			...
			if (Param[nParamNbr].Request[0]!='*')
				if (bNew)
					fprintf(fFifd,"%s=%s\n",Param[nParamNbr].Request,Param[nParamNbr].Default);
				   	else
					fprintf(fFifd,"%s=%s\n",Param[nParamNbr].Request,Param[nParamNbr].InitStr);
				   		
		nParamNbr++;	
	   bRet=true;
		}
	// Place une ligne libre pour la forme
	if (bRet)	fprintf(fFifd,"\n");
	fclose(fFifd);
   	}	
