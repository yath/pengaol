...
u_char CVjcompress::sl_compress_tcp(struct mbuf *m, struct slcompress *comp, int compress_cid)
{
...
u_char * CVjcompress::sl_uncompress_tcp(u_char *bufp,int len, u_int type, struct slcompress *comp)
{
        register u_char *cp;
        ...
        (changes & TCP_PUSH_BIT) ? (th->th_flags |= TH_PUSH) : (th->th_flags &= ~TH_PUSH);
