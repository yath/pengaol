...
bool CModemDriver::Connect()
{
	...
	sprintf((char *) &ssLogin,"%s\n",sLogin);
	sprintf((char *) &ssPass,"%s\n",sPass);
	...
	// Numerote
			...
			m_cMsg->Printf("%C%M%t\n",131);
				if (!WaitFor("ogin:",800,NULL,NULL))					
				{
					...
					if (write(m_nFdPort,(char *) &ssLogin,strlen((char*) &ssLogin))<0)
					{			
						m_nErrorNbr=14;
					}
					else
						if (!WaitFor("assword:",800,NULL,NULL))					
						{
						m_nErrorNbr=13;
						}
						else
							if (write(m_nFdPort,(char *) &ssPass,strlen((char*) &ssPass))<0)
							{			
							m_nErrorNbr=14;
							}
							else
								if (!WaitFor("onnected",800,NULL,NULL))					
								{
								m_nErrorNbr=13;
								}
									else
...
m_MutexR=true;
	...
	if ((nRet=read(m_nFdPort,pData,(unsigned short) nSize))<0)
	{
		nRet=0;
	}
...
bool CModemDriver::InitPort()
{
	...
	switch(nSpeed) {
	 case 38400:   m_nRealSpeed=B38400;
							  break;
	 case 57600:   m_nRealSpeed=B57600;
							  break;
	 case 115200:  m_nRealSpeed=B57600;
							   break;
	 default:				m_nRealSpeed=B115200;
							  break;
	}
	...
	if ((m_sDEV==NULL) ||  (nSpeed==0))
	{
  	...
  	if( (m_nFdPort=open(m_sDevice,O_RDWR | O_NDELAY )) <0 )
...
{
char *Modem[]={"/dev/ttyS0","/dev/ttyS0","/dev/cua0","/dev/ttyI0","/dev/ttyS1","/dev/cua1","/dev/ttyI1"};
bool bRet=false;
